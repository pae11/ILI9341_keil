...
{
	while(DISP_SPI->SR & (1 << 7)) {}; // ожидаем, пока бит BSY (Busy flag) установлен в 1
	DISP_DC_CMD;
	DISP_CS_SELECT;
	DISP_SPI->DR = Command;
	while(!(DISP_SPI->SR & (1 << 1))) {}; // ожидаем, пока бит TXE (Transmit buffer empty) не станет 1
	DISP_CS_UNSELECT;
}
...
void ILI9341_Draw_Colour_Burst(uint16_t Colour, uint32_t Size)
{
	...
	{
		DISP_SPI->DR = (Colour >> 8);
		while(!(DISP_SPI->SR & (1 << 1))) {}; // ожидаем, пока бит TXE (Transmit buffer empty) не станет 1  // ожидаем, пока бит TXE (Transmit buffer empty) не станет 1
		DISP_SPI->DR = Colour;
		while(!(DISP_SPI->SR & (1 << 1))) {}; // ожидаем, пока бит TXE (Transmit buffer empty) не станет 1
		Size--;
	}
